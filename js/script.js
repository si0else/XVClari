...
document.addEventListener("DOMContentLoaded", () => {
  ...
  enterButton.addEventListener("click", () => { opening.classList.add("hidden"); document.body.classList.remove("is-locked"); setMusicState(true); startMusic(); document.querySelectorAll(".reveal").forEach((el, i) => setTimeout(() => el.classList.add("visible"), i * 90)); });
  musicButton.addEventListener("click", () => { const shouldPlay = !musicButton.classList.contains("playing"); setMusicState(shouldPlay); sendPlayerCommand(shouldPlay ? "playVideo" : "pauseVideo"); });
});
